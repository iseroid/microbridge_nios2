...
boolean max3421e_reset(void)
{
	...
	while( 1 ) {
		if( max3421e_read(MAX_REG_USBIRQ) & bmOSCOKIRQ ) {
			return true;
		}
...
int max3421e_powerOn( void )
{
	...
	if( max3421e_reset() == false ) {
		alt_putstr("Error: OSCOKIRQ failed to assert\n");
		return -1;
	}

	// Configure host operation.
	// set pull-downs, Host, Separate GPIN IRQ on GPX
	max3421e_write( MAX_REG_MODE, bmDPPULLDN | bmDMPULLDN | bmHOST | bmSEPIRQ );
	//connection detection
	max3421e_write( MAX_REG_HIEN, bmCONDETIE | bmFRAMEIE );
	...
	max3421e_write( MAX_REG_HIRQ, bmCONDETIRQ );
...
{
	uint8_t rcode = 0;
	uint8_t gpx_int = IORD_ALTERA_AVALON_PIO_DATA( PIO_IN_BASE );
