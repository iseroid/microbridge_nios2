...
int usbInit( void )
{
	...
	usbInitEndPoint( &(deviceTable[0].control), 0 );
 ...
 */
void usbInitEndPoint( usb_endpoint * endpoint, uint8_t address )
{
 ...
 */
int usbInitDevice( usb_device * device, int configuration )
{
	char buf[4];
	uint8_t rcode;

	// Set the configuration for this USB device.
	rcode = usbSetConfiguration( device, configuration );
	if( rcode < 0 ) return rcode;

	// Get the first supported language.
	rcode = usbGetString( device, 0, 0, 4, buf );
	if( rcode < 0 ) return rcode;
 ...
 */
void usbSetEventHandler( usb_eventHandler * handler )
{
...
void usbPoll( void )
{
	...
	case USB_DETACHED_SUBSTATE_INITIALIZE:
		...
		for( i = 1; i < USB_NUMDEVICES; i++ ) {
			if( deviceTable[i].active ) {
				fireEvent( &(deviceTable[i]), USB_DISCONNECT );
			}
		}
	...
	case USB_ATTACHED_SUBSTATE_GET_DEVICE_DESCRIPTOR_SIZE:
		...
		rcode = usbGetDeviceDescriptor( &deviceTable[0], &deviceDescriptor );
		if( rcode == 0 ) {
 ...
 */
int usbGetString(usb_device * device, uint8_t index, uint8_t languageId, uint16_t length, char * str)
{
	uint8_t stringLength = 0;
	int i, ret = 0;

    // Get string length;
	ret = controlRequest(device, bmREQ_GET_DESCR, USB_REQUEST_GET_DESCRIPTOR, index, USB_DESCRIPTOR_STRING, languageId, sizeof(uint8_t), &stringLength);
    if (ret<0) return -1;
	...
	// Get the whole thing.
	ret = controlRequest(device, bmREQ_GET_DESCR, USB_REQUEST_GET_DESCRIPTOR, index, USB_DESCRIPTOR_STRING, languageId, stringLength, (uint8_t *)str);
    if (ret<0) return -2;
 ...
 * @return number of bytes read, or error code in case of failure.
 */
int usbBulkRead(usb_device * device, uint16_t length, uint8_t * data, boolean poll)
{
	return read(device, &(device->bulk_in), length, data, poll ? 1 : USB_NAK_LIMIT);
}
 ...
 * @return number of bytes read, or error code in case of failure.
 */
int usbBulkWrite(usb_device * device, uint16_t length, uint8_t * data)
{
	return write(device, &(device->bulk_out) , length, data);
}
	...
	uint8_t * data )
{
	if( direction ) {
		...
		return read( device, &(device->control), length, data, USB_NAK_LIMIT );

	} else {
		// OUT transfer
		device->control.sendToggle = bmSNDTOG1;
		return write( device, &(device->control), length, data );
	}
}
...
//		serialPrintf("Setup packet error: 0x%02x\n", rcode);
	...
	if( data != NULL ) {
		rcode = ctrlData( device, direction, length, data );
 ...
 */
int usbGetDeviceDescriptor(usb_device * device, usb_deviceDescriptor * descriptor)
{
	return(controlRequest(device, bmREQ_GET_DESCR, USB_REQUEST_GET_DESCRIPTOR, 0x00, USB_DESCRIPTOR_DEVICE, 0x0000, sizeof(usb_deviceDescriptor), (uint8_t *)descriptor));
}
 ...
 */
int usbGetConfigurationDescriptor(usb_device * device, uint8_t conf, uint16_t length, uint8_t * data)
{
	uint16_t descriptorLength;
	int rcode;

	// Read the length of the configuration descriptor.
	rcode = (controlRequest(device, bmREQ_GET_DESCR, USB_REQUEST_GET_DESCRIPTOR, conf, USB_DESCRIPTOR_CONFIGURATION, 0x0000, 4, data));
	if (rcode) return -1;
	...
	// Read the length of the configuration descriptor.
	rcode = (controlRequest(device, bmREQ_GET_DESCR, USB_REQUEST_GET_DESCRIPTOR, conf, USB_DESCRIPTOR_CONFIGURATION, 0x0000, length, data));
	if (rcode) return -2;
...
static int setAddress(usb_device * device, uint8_t address)
{
    return( controlRequest(device, bmREQ_SET, USB_REQUEST_SET_ADDRESS, address, 0x00, 0x0000, 0x0000, NULL) );
}
 ...
 */
int usbSetConfiguration(usb_device * device, uint8_t configuration)
{
    return( controlRequest( device, bmREQ_SET, USB_REQUEST_SET_CONFIGURATION, configuration, 0x00, 0x0000, 0x0000, NULL) );
}
